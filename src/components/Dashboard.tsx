...
const Dashboard: React.FC<DashboardProps> = ({ onPageChange, recentTransactions }) => {
  // Calculate financial stats from recentTransactions
  const totalBalance = recentTransactions.reduce((sum, tx) => sum + tx.amount, 0);
  ...
  const currentSavings = 8500; // You may update this as dynamic if you want
    ...
    <div className="space-y-8 animate-slide-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="gradient-card shadow-card transition-smooth hover:shadow-elegant">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-success">₹{monthlyIncome.toLocaleString("en-IN")}</div>
        ...
        <Card className="gradient-card shadow-card transition-smooth hover:shadow-elegant">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">₹{monthlyExpenses.toLocaleString("en-IN")}</div>
        ...
        <Card className="gradient-card shadow-card transition-smooth hover:shadow-elegant">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-accent">{savingsProgress.toFixed(0)}%</div>
            <div className="w-full bg-muted rounded-full h-2 mt-2">
              <div className="bg-accent h-2 rounded-full transition-all" style={{ width: `${savingsProgress}%` }} />
            </div>
            <p className="text-xs text-muted-foreground">
              ₹{currentSavings.toLocaleString("en-IN")} of ₹{savingsGoal.toLocaleString("en-IN")}
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="space-y-4">
            {recentTransactions.length === 0 ? (
              ...
              recentTransactions.map((transaction) => (
                <div key={transaction.id} className="flex items-center justify-between p-4 bg-background rounded-lg border border-border">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${transaction.type === "income" ? "bg-success/20" : "bg-destructive/20"}`}>
                      <DollarSign className={`w-5 h-5 ${transaction.type === "income" ? "text-success" : "text-destructive"}`} />
                    </div>
                    <div>
                      <p className="font-medium text-foreground">{transaction.description}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card
          className="gradient-card shadow-card cursor-pointer transition-smooth hover:shadow-elegant"
        ...
        >
          <CardContent className="flex flex-col items-center justify-center p-8 text-center">
            <Receipt className="w-12 h-12 text-warning mb-4" />
            <h3 className="text-lg font-semibold text-foreground mb-2">Scan Receipts</h3>
            <p className="text-sm text-muted-foreground">Extract expenses from receipts</p>
          </CardContent>
        </Card>
      </div>
